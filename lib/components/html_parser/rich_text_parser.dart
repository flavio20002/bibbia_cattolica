...
class HtmlRichTextParser extends StatelessWidget {
  ...
  final _supportedSpecialtyElements = [
    "a",
    "br",
    "table",
    "tbody",
  ...
  Widget build(BuildContext context) {
    ...
    for (var elem in document.getElementsByTagName("sup")) {
      var newString = "";
      elem.text.split('').forEach((ch) => {newString += superscript[ch] ?? ch});
      elem.text = newString;
    }
    ...
    widgetList.forEach((dynamic w) {
      if (w is BlockText) {
        if (w.child.textSpan.text == null) return;
